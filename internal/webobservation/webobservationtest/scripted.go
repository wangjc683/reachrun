// Package webobservationtest supplies the deterministic Observer adapter used
// by tests of Web-observation consumers.
...
// panics so a test cannot silently manufacture unstated Web evidence.
type Scripted struct {
	mu      sync.Mutex
	results []webobservation.Result
	calls   []Call
}

var _ webobservation.Observer = (*Scripted)(nil)

// New returns a scripted adapter with a defensive copy of results.
func New(results ...webobservation.Result) *Scripted {
	return &Scripted{results: append([]webobservation.Result(nil), results...)}
}

// Observe implements webobservation.Observer.
func (s *Scripted) Observe(
	...
	if len(s.results) == 0 {
		panic("webobservationtest: unexpected Observe call")
	}
	result := s.results[0]
