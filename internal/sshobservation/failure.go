...
func classifyUnconfirmedReason(err error) UnconfirmedReason {
	switch {
	case errors.Is(err, errInvalidIdentification):
		return UnconfirmedInvalidIdentification
	case errors.Is(err, errPreambleLimit):
		return UnconfirmedPreambleLimit
	case isTimeout(err):
	...
	default:
		return UnconfirmedExchangeFailure
	}
}
