...
ReachRun 处于 **Phase 0 in progress / address-family condition evidence slice implemented** 阶段。
...
- `reachrun family-conditions`、`reachrun resolve`、`reachrun resolver-inventory`、`reachrun dns-observe`、`reachrun dns-https-path`、`reachrun web-path`、`reachrun web-recheck`、`reachrun web-observe`、`reachrun ssh-observe` 与 `reachrun tls-observe` 十条命令；
- Public Web Path、Web、SSH 与 TLS 共用的公网字面量 IP 安全策略，以及共用的 Web hostname 规范化策略；
- macOS、Windows 与 Linux 原生 resolver contract test、Address Family Conditions、受控 DNS/DNS HTTPS Path/Web/Web Candidate Recheck/SSH/TLS contract test，以及三平台 GitHub Actions workflow；
- 当前实现架构和可复现开发命令。
...
前九条垂直切片已完成实现。当前代码和本地测试证明 envelope、System Resolution、Resolver Inventory、Address Family Conditions、DNS Observation、DNS HTTPS Path、Web Observation、Web Candidate Recheck、SSH Observation、TLS Observation 与 Public Web Path contract；GitHub-hosted runner 状态和真实设备网络行为仍必须现场核验，不能由本快照推断。
...
前九份实现切片已完成。下一份切片按 PRD §19 建立受控重试与批次取消的最小编排证据，并继续覆盖三平台策略场景；在相关网络模式关闭后还需复测当前 macOS 的公开 Web 路径。Address Family Conditions 与目标 probe 的正式中性 assessment，以及系统候选、两个独立参考来源、Web Candidate Recheck 与 CDN/GeoDNS 排除证据的组合，都留给后续 `checkrun`/`assessment`，不得从手工 CLI 标签或单次 no-route 直接生成归因。
