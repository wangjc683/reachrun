...
- `0`：probe 成功取得合法证据，或 aggregate/path 报告 `completed`；DNS 的 NXDOMAIN、NODATA、SERVFAIL 等合法响应、DNS HTTPS Path 的 `unsupported_service_mode`、Web Candidate Recheck 中两侧失败但受控尝试已完成、Web 的任意合法 `4xx/5xx` HTTP 响应，以及 SSH 端口可达但 identification 未确认，也属于成功观测；
...
UDP、TCP、DoH、DNS HTTPS 路径、Web、公开 Web 路径、Web 候选复核与 SSH contract test 使用进程内受控服务或 scripted adapter，不依赖公共 resolver 或公网站点是否可达。CI 不对 Cloudflare/Google 或示例 Web/SSH endpoint 发真实请求，避免把外部网络波动变成构建结果。
